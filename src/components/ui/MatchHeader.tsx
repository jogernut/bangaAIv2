          ...
          {mode === 'model' ? (
            ...
            <div className={`grid gap-3 ${mode === 'market' ? 'grid-cols-[1fr_1fr_1fr_1fr] max-w-[400px]' : 'grid-cols-4'}`}>
              {models.map((model) => (
                <div
                  key={model}
                  className={`bg-gray-800/70 rounded-md text-center border border-gray-700/50 flex flex-col items-center justify-center ${
                    mode === 'market' ? 'px-3 py-2 min-w-[80px]' : 'px-2 py-2'
                  }`}
                >
